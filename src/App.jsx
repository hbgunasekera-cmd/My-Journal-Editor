    ...
    return [...filtered].sort((a, b) => {
      if (sortBy === 'distance') {
        const distA = L.latLng(sortCenter.lat, sortCenter.lng)
          .distanceTo(L.latLng(a.latitude, a.longitude));
        const distB = L.latLng(sortCenter.lat, sortCenter.lng)
          .distanceTo(L.latLng(b.latitude, b.longitude));
        return distA - distB;
      }
...
`, { closeButton: false, className: 'custom-map-popup' });
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50">
      {/* GLOBAL HEADER */}
      <header className="h-16 bg-white border-b flex items-center justify-between px-6 z-[1001] shrink-0 shadow-sm">
        <div className="flex items-center gap-6">
          <h1 className="text-sm font-black uppercase tracking-tighter text-indigo-900 hidden sm:block">My Journal Admin</h1>
          <nav className="flex bg-slate-100 p-1 rounded-xl">
            {['places', 'map', 'dashboard'].map(t => (
              <button key={t} onClick={() => setActiveTab(t)} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${activeTab === t ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{t}</button>
            ))}
          </nav>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-wrap items-center gap-3 p-4 bg-white border-b sticky top-0 z-[1000]">
            ...
            <div className="relative flex items-center">
              ...
              />
              {/* Note: Google automatically renders the dropdown list here */}
            </div>
                  ...
                  return (
                    <div key={p.id} className="bg-white rounded-[1.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col group relative hover:shadow-md transition-all">
                      ...
                      <div className="aspect-video bg-slate-200 relative shrink-0 overflow-hidden">
                        ...
                        <button onClick={() => updatePlaceField(p.id, 'status', p.status === 'done' ? 'pending' : 'done')} className={`absolute top-2 right-2 px-3 py-1 rounded-full text-[8px] font-black uppercase shadow-md transition-all active:scale-95 ${p.status === 'done' ? 'bg-emerald-500 text-white' : 'bg-white/90 text-orange-500'}`}>
